...
export default function HelpCenter() {
  ...
  return (
    <div className="py-12 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="space-y-4">
          {guides.map((item, index) => {
            ...
            return (
              <div
                key={item.id || index}
                className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-xs hover:border-indigo-200 transition-all"
              >
                <button
                  onClick={() => setOpenIndex(isOpen ? -1 : index)}
                  className="w-full text-left p-5 sm:p-6 flex items-center justify-between gap-4 font-bold text-slate-800 text-xs sm:text-sm hover:text-indigo-600 transition-colors cursor-pointer"
                >
      ...
      {/* Support Box */}
      <div className="bg-slate-950 rounded-3xl p-8 sm:p-10 text-white shadow-xl flex flex-col sm:flex-row items-center justify-between gap-6 border border-slate-800">
        <div>
        ...
        <Link
          to="/contact-us"
          className="btn-primary text-xs py-3 px-6 whitespace-nowrap shadow-md"
        >
